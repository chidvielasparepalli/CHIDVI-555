...
MEDIA_FILE = CONFIG_DIR / "media.json"

_INTERFACE_VIDEO_NAMES = (
    "BackgroundWidget", "interface_video.mov", "interface_video.webm",
    "jarvis_interface.mp4", "jarvis_interface.mov", "jarvis_interface.webm",
    "wake_interface.mp4", "hud.mp4", "interface.mp4",
)
_BACKGROUND_AUDIO_NAMES = (
    "background_song.mp3", "background_song.wav", "background_song.m4a",
...
def _configured_media_path(key: str) -> Path | None:
    try:
...
def _first_media_file(key: str, names: tuple[str, ...]) -> Path | None:
    configured = _configured_media_path(key)
    if configured:
        return configured
    for name in names:
        path = MEDIA_DIR / name
...
def _background_volume() -> float:
    try:
        data = json.loads(MEDIA_FILE.read_text(encoding="utf-8"))
        return max(0.0, min(1.0, float(data.get("background_volume", 0.35))))
    except Exception:
        return 0.35
...
class VideoHud(QWidget):
    def __init__(self, video_path: Path, parent=None):
        ...
        self._video.setStyleSheet(f"background: {C.BG};")

        self._status = QLabel(self)
        self._status.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._status.setFont(QFont("Courier New", 11, QFont.Weight.Bold))
        self._status.setStyleSheet(
            "background: rgba(0, 6, 10, 150);"
            f"color: {C.PRI};"
        ...
        self._player.errorOccurred.connect(lambda *_: self._status.setText("VIDEO UNAVAILABLE"))

        self._status_tmr = QTimer(self)
        self._status_tmr.timeout.connect(self._update_status)
        self._status_tmr.start(250)
        self._update_status()
    ...
    def resizeEvent(self, event):
        ...
        h = 46
        self._status.setGeometry(0, max(0, self.height() - h), self.width(), h)
        self._status.raise_()
    ...
    def change_video(self, video_path: Path):
        self._player.stop()
        self._player.setSource(QUrl.fromLocalFile(str(video_path)))
        self._player.play()
    ...
    def _update_status(self):
        if self.muted:
            text, color = "MICROPHONE MUTED", C.MUTED_C
        elif self.speaking:
            text, color = "SPEAKING", C.ACC
        elif self.state == "LISTENING":
            text, color = "LISTENING", C.GREEN
        elif self.state == "THINKING":
            text, color = "THINKING", C.ACC2
        else:
            text, color = self.state, C.PRI
...
class BackgroundSong(QObject):
    def __init__(
        ...
        if not audio_path or not _HAS_QT_MULTIMEDIA:
            QTimer.singleShot(700, self._notify_finished)
            return
        try:
            self._audio = QAudioOutput(self)
            ...
            self._player.mediaStatusChanged.connect(self._stop_at_end)
            QTimer.singleShot(300, self._play_once)
        except Exception:
            self._player = None
            self._audio = None
            QTimer.singleShot(700, self._notify_finished)
    ...
    def _play_once(self):
        if not self._player or self._has_played:
            return
        self._has_played = True
        self._player.setPosition(0)
        self._player.play()

    def _stop_at_end(self, status):
...
class WorkMusicPlayer(QObject):
    def __init__(
        self,
        audio_path: Path | None,
        normal_volume: float = 0.30,
        duck_volume: float = 0.055,
        parent=None,
    ):
        super().__init__(parent)
        self._player = None
        self._audio = None
        self._normal_volume = max(0.0, min(1.0, normal_volume))
        self._duck_volume = max(0.0, min(1.0, duck_volume))
        self._ducked = False
        ...
        try:
            self._audio = QAudioOutput(self)
            self._audio.setVolume(self._normal_volume)
            self._player = QMediaPlayer(self)
            self._player.setAudioOutput(self._audio)
            self._player.setSource(QUrl.fromLocalFile(str(audio_path)))
            self._player.mediaStatusChanged.connect(self._loop_forever)
        except Exception:
            self._player = None
            self._audio = None
    ...
    def start(self):
        if not self._player or not self._start_allowed:
            return
        self._apply_volume()
        self._player.play()

    def duck(self, value: bool):
        self._ducked = bool(value)
        self._apply_volume()

    def _apply_volume(self):
        if not self._audio:
            return
        self._audio.setVolume(self._duck_volume if self._ducked else self._normal_volume)

    def _loop_forever(self, status):
        if not self._player:
            return
        if status == QMediaPlayer.MediaStatus.EndOfMedia:
            self._player.setPosition(0)
...
class MicLevelAnalyzer(QWidget):
    """Real-time microphone level analyzer with waveform history and peak indicator."""

    def __init__(self, label: str = "MIC", color: str = C.GREEN, parent=None):
        super().__init__(parent)
        self._label = label
        self._color = color
        self._db = -100.0
        self._peak = -100.0
        self._peak_decay = -100.0
    ...
    def update_level(self, db: float):
        self._db = max(-100.0, min(0.0, db))
        self._history.append(self._db)
        ...
        if self._db > self._peak:
            ...
            self._peak_timer += 1
            if self._peak_timer > 15:
                self._peak = max(self._db, self._peak - 0.8)
        self.update()
    ...
    def paintEvent(self, _):
        ...
        label_h = 14
        bar_area_top = pad + label_h + 2
        bar_area_h = H - bar_area_top - pad - 4
        bar_area_w = W - pad * 2

        # label
        p.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
        p.setPen(QPen(qcol(C.TEXT_DIM), 1))
        p.drawText(QRectF(pad + 4, pad + 1, 60, label_h),
                   Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter,
                   f"◈ {self._label}")

        # dB value
        if self._db > -100:
            db_text = f"{self._db:+.0f} dB"
        else:
            db_text = "--- dB"
        p.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        p.setPen(QPen(qcol(self._color) if self._db > -90 else qcol(C.TEXT_DIM), 1))
        p.drawText(QRectF(pad + 40, pad + 1, bar_area_w - 44, label_h),
                   Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter,
                   db_text)

        # waveform history
        wave_h = int(bar_area_h * 0.55)
        wave_top = bar_area_top
        wave_bottom = wave_top + wave_h
        n = len(self._history)
        if n > 1:
            step_x = bar_area_w / (self._max_history - 1)
            for i in range(n - 1):
                x0 = pad + i * step_x
                x1 = pad + (i + 1) * step_x
                v0 = max(0.0, (self._history[i] + 100) / 100)
                v1 = max(0.0, (self._history[i + 1] + 100) / 100)
                y0 = wave_bottom - v0 * wave_h
                y1 = wave_bottom - v1 * wave_h
                intensity = max(0, min(255, int(180 * v1)))
                col = qcol(self._color, intensity)
                p.setPen(QPen(col, 2))
                p.drawLine(QPointF(x0, y0), QPointF(x1, y1))

        # level bar
        bar_top = wave_bottom + 4
        bar_h = max(4, bar_area_h - wave_h - 8)
        bar_w = bar_area_w
        fill_pct = max(0.0, (self._db + 100) / 100)
        fill_w = int(bar_w * fill_pct)
        p.setBrush(QBrush(qcol(C.BAR_BG)))
        p.setPen(Qt.PenStyle.NoPen)
        p.drawRoundedRect(QRectF(pad, bar_top, bar_w, bar_h), 2, 2)
        if fill_w > 0:
            if fill_pct > 0.85:
                bar_col = qcol(C.RED)
            elif fill_pct > 0.65:
                ...
                bar_col = qcol(self._color)
            p.setBrush(QBrush(bar_col))
            p.drawRoundedRect(QRectF(pad, bar_top, fill_w, bar_h), 2, 2)

        # peak indicator
        if self._peak > -90:
            peak_x = pad + int(bar_w * max(0.0, (self._peak + 100) / 100))
            p.setPen(QPen(qcol(C.RED, 220), 2))
            p.drawLine(QPointF(peak_x, bar_top - 1), QPointF(peak_x, bar_top + bar_h + 1))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self.setWindowTitle(f"{_display} — CHIDVI 555")
        ...
        # Center column: VRM renderer + resizable content panel via QSplitter
        if _HAS_WEBENGINE:
            ...
            self.hud.setStyleSheet("background: #00060a;")
            self.hud.loadStarted.connect(
                lambda: self._log_sig.emit("SYS: Renderer page load started.")
            )
            self.hud.loadFinished.connect(self._on_renderer_loaded)
        else:
        ...
        if _HAS_WEBENGINE:
            ...
            self.hud.load(QUrl(f"http://127.0.0.1:5173/?avatar={avatar}"))

        self._work_music = WorkMusicPlayer(
            _first_media_file("work_background_music", _WORK_AUDIO_NAMES),
            0.30, 0.055, self,
        )
        self._background_song = BackgroundSong(
            _first_media_file("background_song", _BACKGROUND_AUDIO_NAMES),
            _background_volume(), self,
            on_finished=self._start_work_music_after_wake,
        )

    def _on_renderer_loaded(self, ok: bool):
        if ok:
            self._log_sig.emit("SYS: Renderer page load finished -> OK.")
        else:
            self._log_sig.emit("SYS: Renderer page load FAILED -> retrying in 2s...")
            QTimer.singleShot(2000, self._reload_renderer)

    def _reload_renderer(self):
        if not _HAS_WEBENGINE or not hasattr(self.hud, "page"):
            return
        try:
            current_url = self.hud.url().toString()
            if current_url:
                self._log_sig.emit(f"SYS: Renderer reload -> {current_url}")
                self.hud.reload()
            else:
                from core.personality_manager import get_personality
                avatar = "Hinata.vrm" if get_personality() == "HINATA" else "Chidvi.vrm"
            ...
            self._log_sig.emit(f"SYS: Renderer reload error -> {e}")
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("CHIDVI 555", C.PRI_DIM))
        lay.addSpacing(8)
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        audio_hdr = QLabel("◈ AUDIO")
        audio_hdr.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
        audio_hdr.setStyleSheet(f"color: {C.PRI}; background: transparent; "
                                f"border-bottom: 1px solid {C.BORDER}; padding-bottom: 4px;")
        lay.addWidget(audio_hdr)

        self._mic_analyzer = MicLevelAnalyzer("MIC", C.GREEN)
        lay.addWidget(self._mic_analyzer)

        self._spk_analyzer = MicLevelAnalyzer("SPK", C.ACC2)
        lay.addWidget(self._spk_analyzer)
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        ...
        self.setWindowTitle(f"{display} — CHIDVI 555")
    ...
    def _send(self):
        ...
        if hasattr(self, "_work_music"):
            self._work_music.start()
            self._work_music.duck(False)
        if self.on_text_command:
            threading.Thread(target=self.on_text_command, args=(txt,), daemon=True).start()

    def _apply_state(self, state: str):
        if hasattr(self, "_state_sounds"):
            self._state_sounds.apply_state(state, self._muted)

        if hasattr(self, "_work_music"):
            if state == "SPEAKING":
                self._work_music.duck(True)
            else:
                self._work_music.duck(False)
    ...
    def _start_work_music_after_wake(self):
        if hasattr(self, "_work_music"):
            self._work_music.allow_start()
            self._work_music.duck(getattr(self.hud, "speaking", False))

    def switch_theme(self, personality: str):
        if theme_manager:
            theme_manager.switch(personality)
        if hasattr(self, "_avatar_switch_sig"):
            avatar = "Hinata.vrm" if personality == "HINATA" else "Chidvi.vrm"
            self._avatar_switch_sig.emit(avatar)
    ...
    def set_audio_diagnostics_provider(self, provider):
        """Set the audio diagnostics provider callback."""
        self._audio_diagnostics_provider = provider

    def _update_audio_diagnostics(self):
        if not self._audio_diagnostics_provider:
            return
        try:
            diag = self._audio_diagnostics_provider()
            if diag:
                mic_db = diag.get("mic_db", -100.0)
                spk_db = diag.get("spk_db", -100.0)
                self._mic_analyzer.update_level(mic_db)
                self._spk_analyzer.update_level(spk_db)
        except Exception:
            pass
...
class JarvisUI:
    ...
    def start_work_music(self):
        if hasattr(self._win, "_work_music"):
            self._win._work_music.start()
            self._win._work_music.duck(False)

    def duck_work_music(self, value: bool):
        if hasattr(self._win, "_work_music"):
            self._win._work_music.duck(value)
