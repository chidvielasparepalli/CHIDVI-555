...
class _BrowserSession:
        ...
        try:
            ...
            if self._context.pages:
                ...
                if self._context.pages:
                    ...
                    print("Pages:", len(self._context.pages))
    ...
    async def go_to(self, url: str) -> str:
        ...
        print("Pages before:", len(self._context.pages))
        result_url = await _do_goto(page)
        print("Result URL:", result_url)
        print("Pages after:", len(self._context.pages))
        for i, p in enumerate(self._context.pages):
            print(f"Page {i}: {p.url}")
        print("====== GO_TO END ======")
        if result_url in ("about:blank", "", None):
            await asyncio.sleep(1)

            if page.url not in ("about:blank", "", None):
                result_url = page.url
